// Relay
// A digital output class that helps to set digital pins on and off
// it also handles setting different flashing patterns, 
// and is planed to be extended to handle also dimming functionaity
//
// Typlical Actualtors are controled via relays, FET transistors, tricas, ... 
// 
...
class Relay {
  private:
    ...
    String  _name               = "relay";      // Needed for all sensors
...
//      this->_report = report;
    ...
    // Alterative constructor
    void begin(byte pin = LED_BUILTIN, boolean report = false , String name = "relay", unsigned long postingIntervalDelay = 27 * 1000, boolean inverted = false) {
    ...
    void debug(byte pin = LED_BUILTIN, String name = "relay", unsigned long postingIntervalDelay = 27 * 1000, boolean inverted = false) {
      this->_pin = pin;
      this->_name = name;
      this->_postingIntervalDelay = postingIntervalDelay;
      this->_inverted = inverted;
      ...
      _debug = true;
      if (_debug) {
        Serial.println("Relay on pin: " + (String) _pin + " , _postingIntervalDelay: " + (String) _postingIntervalDelay);
      }      
      mqttJSON[_name ] = "off";
      _postingIntervalTimer = millis();
    }
    ...
    // Alterative constructor that enables debug printouts
    void debug(byte pin = LED_BUILTIN, boolean report = false , String name = "relay", unsigned long postingIntervalDelay = 27 * 1000, boolean inverted = false) {
    ...
    void on()
    {
      ...
      if (_debug) {Serial.println("Relay on pin: " + (String) _pin + " is: On");}
      _currentState = HIGH;
    }
    ...
    void off()
    {
      ...
      if (_pin != 0 ) { digitalWrite(_pin, _LOW); }
      if (_debug) {Serial.println("Relay on pin: " + (String) _pin + " is: Off");}
      _currentState = LOW;
    }
    ...
    void flash()
    {
      ...
      if (_pin != 0 ) { digitalWrite(_pin, _HIGH); }
      if (_debug) {Serial.println("Relay on pin: " + (String) _pin + " is: Flashing");}
      _currentState = HIGH;
    }
    ...
    void quickflash()
    {
      ...
      if (_pin != 0 ) { digitalWrite(_pin, _HIGH); }
      if (_debug) {Serial.println("Relay on pin: " + (String) _pin + " is: Quick Flashing");}
      _currentState = HIGH;
    }
    ...
    void slowflash()
    {
      ...
      if (_pin != 0 ) { digitalWrite(_pin, _HIGH); }
      if (_debug) {Serial.println("Relay on pin: " + (String) _pin + " is: Slow Flashing");}
      _currentState = HIGH;
    }
...
//        if ((millis() % _periodicity - _periodicity/2) < _periodicity/2 ) // Change to > _periodicity
      ...
      if (_currentState == HIGH ) 
      {
        ...
        if  (((m % _longPeriodicity - _longPeriodicity/2) > _longPeriodicity) && ((m % _shortPeriodicity - _shortPeriodicity/2) > _shortPeriodicity)) {
          if (_currentPinState == LOW) {
            ...
            if (_debug) { Serial.println("Relay on pin: " + (String) _pin + " , called: " + _name + " : now HIGH : Previous state: " + (String) _duration + " ms"); }
            _changetime = m;
          ...
          if (_currentPinState == HIGH) {
            ...
            if (_debug) { Serial.println("Relay on pin: " + (String) _pin + " , called: " + _name + " : now LOW : Previous state: " + (String) _duration + " ms"); }
            _changetime = m;
    ...
    boolean state() {
      if (_debug) {
        //Serial.println("Relay on pin: " + (String) _pin + " is: " + (String) _currentState);
      }
